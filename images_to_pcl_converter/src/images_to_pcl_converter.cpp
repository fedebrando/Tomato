...
private:
    void imageCallback(const sensor_msgs::msg::Image::SharedPtr rgb_msg, const sensor_msgs::msg::Image::SharedPtr depth_msg)
    {
        ...
        // Save RGB image every SAVE_FREQUENCY frames
        frame_counter_++;
        if (frame_counter_ % SAVE_FREQUENCY == 0)
        {
            try
            {
                std::stringstream filename;
                filename << SAVE_DIRECTORY << "/frame_" << frame_counter_ << ".ppm";
                std::ofstream ofs(filename.str(), std::ios::binary);

                if (!ofs)
                {
                    RCLCPP_ERROR(this->get_logger(), "Failed to open file: %s", filename.str().c_str());
                    return;
                }
                ...
                // Write raw RGB data
                ofs.write(reinterpret_cast<const char *>(rgb_msg->data.data()), rgb_msg->data.size());

                ofs.close();

                RCLCPP_INFO(this->get_logger(), "Saved RGB image to: %s", filename.str().c_str());
            }
    ...
    // Frame counter for periodic image saving
